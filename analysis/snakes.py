...
class SnakesFactory:
    ...
    def getClusters(self,plot=False):
        ...
        if self.options.processing == 1:
            ...
            print(f"Not apply preprocessing: {self.rebin}")
            edges = self.ct.arrrebin(self.image_fr_zs,self.rebin)
            edcopyTight = edges.copy()
        # make the clustering with DBSCAN algo
        # this kills all macrobins with N photons < 1
        print(f'Número de pontos após processamento {edcopyTight.shape[0]}')
        ...
        print(f'Shape pre clustering {X.shape}')
